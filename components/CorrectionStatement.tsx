...
import { DeductorDetails } from './DeductorDetails.tsx';
import { ChallanDetails } from './ChallanDetails.tsx';
import { DeducteeDetails } from './DeducteeDetails.tsx';
import { PanUpdate } from './PanUpdate.tsx';
import { SalaryDetails } from './SalaryDetails.tsx';
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      setTimeout(() => {
        setMetaData({
          ...
          sourceFormat: extension === 'fvu' ? 'NSDL FVU V8.7' : 'Excel/CSV Consolidated'
        });
        setFileLoaded(true);
        setIsScanning(false);
      }, 1200);
                    ...
                    <div className="flex flex-col">
                      ...
                      {fileLoaded && (
                        <span className="text-[10px] font-black text-emerald-600 uppercase tracking-tighter">
                          Format Detected: {metaData.sourceFormat}
                        </span>
                      )}
                    </div>
                 </div>
              ...
              <div className="grid lg:grid-cols-3 gap-6 items-center">
                    ...
                    <div className="flex items-center gap-5">
                      ...
                      <div className="flex-grow overflow-hidden">
                        <p className="text-base font-bold text-slate-800 truncate">
                          {filePath || "Import latest Consolidated Excel/CSV or validated .FVU file"}
                        </p>
                        <p className="text-xs text-slate-500 mt-1 flex items-center gap-2">
                          <ShieldCheck size={12} className="text-emerald-500" />
                          Validated for NSDL RPU V8.7 • Secure TRACES Integration
                        </p>
                      </div>
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-center h-full">
                   ...
                   <p className="text-xs text-slate-500 leading-relaxed font-medium">
                     The system now supports <span className="text-emerald-600 font-bold">.fvu</span> file formats. Ensure the file is not corrupted before upload.
                   </p>
                   {fileLoaded && (
                     <button 
